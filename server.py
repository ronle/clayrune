...
# load_project (projects family, 1.11) + _cf_session_nonce_from_request
# (remote family, 1.7). _handle_push_signal stays importable here for the
...
_bp_push_mobile.wire(
    ...
    cf_session_nonce_fn=_cf_session_nonce_from_request,
    get_remote_provider_fn=_get_remote_provider,
)
...
# Routes, helpers, and the gate body moved verbatim. wire() late-binds the two
# deps that have not extracted yet (_DATA_ROOT path; _is_cf_tunneled_request,
# remote family → 1.7). The before_request handler stays registered on `app`
...
_bp_local_auth.wire(
    ...
    is_cf_tunneled_request=_is_cf_tunneled_request,
)
...
_bp_system.wire(load_project_fn=load_project, load_projects_fn=load_projects,
                ...
                is_cf_tunneled_request_fn=_is_cf_tunneled_request,
